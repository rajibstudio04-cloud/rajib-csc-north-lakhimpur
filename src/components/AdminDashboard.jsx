...
export const AdminDashboard = ({ onLogout }) => {
  ...
  const [adminTab, setAdminTab] = useState('applications'); // 'applications' or 'wallet'
  ...
  const filteredApps = (applications || []).filter((app) => {
    ...
    const matchesSearch = id.includes(term) || name.includes(term) || phone.includes(term);
  ...
  const handleSendWhatsAppNotification = () => {
    if (!selectedApp) return;
    const url = webhookService.getCitizenUpdateWhatsAppLink(selectedApp, editStatus, editRemarks);
    window.open(url, '_blank');
    showToast(`WhatsApp message opened for +91 ${selectedApp.phone}`, 'success');
  };
  ...
  return (
    <div className="space-y-5 animate-in fade-in">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <h1 className="text-lg sm:text-xl font-black text-white tracking-tight leading-tight">
                Rajib CSC <span className="text-cyan-300 font-extrabold">Admin</span>
              </h1>
            ...
            <p className="text-xs text-slate-300 font-medium tracking-wide">
              VLE Operator Dashboard • North Lakhimpur, Assam
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200/90 shadow-sm space-y-1">
          <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">Processing</span>
          <div className="flex items-center justify-between">
      ...
      <div className="bg-white p-3.5 sm:p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col sm:flex-row justify-between items-center gap-3">
        ...
        <div className="flex items-center gap-1.5 flex-wrap w-full sm:w-auto">
          {['All', 'Submitted', 'Pending', 'Processing', 'Completed'].map((st) => (
            <button
              key={st}
              ...
              {filteredApps.length > 0 ? (
                filteredApps.map((app) => (
                  <tr key={app.id} className="hover:bg-slate-50 transition-colors">
                    <td className="p-3 font-mono font-bold text-csc-navy">{app.id}</td>
                    <td className="p-3">
                    ...
                    <td className="p-3 text-right">
                      <button
                        ...
                        className="bg-csc-navy hover:bg-csc-lightBlue text-white font-extrabold px-3 py-1.5 rounded-lg text-xs flex items-center gap-1 ml-auto shadow-xs cursor-pointer"
                      >
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                ...
                <div className="sm:col-span-2">
                  <span className="text-slate-500 block">Lakhimpur Address:</span>
              ...
              <form onSubmit={handleSaveStatus} className="bg-blue-50/70 p-5 rounded-xl border border-blue-200 space-y-4 text-xs">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="font-semibold text-slate-700 block mb-1">Official E-Certificate File</label>
                    <input
                      type="text"
                      value={issuedDocName}
                ...
                <div>
                  <label className="font-semibold text-slate-700 block mb-1">Operator Notes / Government Tracking Reference</label>
                  <textarea
                    rows="2"
                    value={editRemarks}
                    onChange={(e) => setEditRemarks(e.target.value)}
                    placeholder="Enter circle officer approval ref or remarks for citizen"
